...
Основные особенности:
    • У вычислений ламбертовская диаграмма: I(θ) = I0 * cos(θ)
    • Освещённость вычисляется через E = I0 * zL^2 / r^4
    • Присутствует нормировка изображения по области интереса (круг в плоскости)
    • Реализован графический интерфейс на Tkinter
    • Возможность отображать картинку, горизонтальные и вертикальные срезы
    • Сохранение результатов в PNG

Проект переписан полностью, изменена структура, переменные и логика,
добавлены комментарии для удобства чтения.
"""
...
def calc_light_map(
    ...
    # Приведение типов (минимальная защита)
    area_w = float(area_w)
    area_h = float(area_h)
    grid_w = int(grid_w)
    grid_h = int(grid_h)
...
# ======================================================================
# ==========================  ГРАФИЧНЫЙ ИНТЕРФЕИС  =====================
# ======================================================================

class LightGUI(tk.Tk):
    """
    Основное окно приложения. Полностью переписанная структура,
    хотя функциональность аналогична исходной.
    """
    ...
    def build_interface(self):
        ...
        # Значения по умолчанию (все изменены относительно исходных)
        defaults = {
            "Ширина W (мм)": 2000,
            "Высота H (мм)": 2000,
            "Сетка Wres": 400,
